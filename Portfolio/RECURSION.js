...
/**
 * Function that generates an array of letters ranging from charA to charZ. The passed in values don't necessarily need to be 'a' and 'z' 
 * but for this activity they should always be the default values.
...
const searchForAlphabeticalIndex = ([stringToFindIndexForFirstLetter], alphabetArray = generateAlphabetArray()) => {
    // The binarySearchResults and linearSearchResults variables are incremented every time you iterate the binary and linear searches.
    //As an iteration comes with the cost of computational time,
    ...
    let recursiveBinarySearch = (array, letter, startIndex, endIndex) => {
        ...
        // establish base case for not finding the letter in the given array, if you are on an iteration where the startIndex 
        //is greater than the endIndex, first set the binarySearchResults variable to be -1 and then return false
        if (startIndex > endIndex) {
            binarySearchResults = -1
            return false;
        }
        //find middle index between the given start and end, hint: use Math.floor(your calculation for the middle index 
        //between start and end) as that rounds down to the nearest integer, you are not searching decimals
